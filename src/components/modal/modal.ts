...
//Rendering modal/leave with money page
export const createLeaveWithMoney = () => {
  ...
  heading.textContent = "Are you sure you want to leave?";

  const p = document.createElement("p");
  p.classList.add("description-text");
  p.textContent = `Leaving now secures your winnings, but continuing could bring you even greater rewards—or risks. Will you play it safe or take the chance? The choice is yours! You would get a grand total of ${totalPricePot.totalPrice}`;

  const removeModal = document.createElement("button");
  removeModal.classList.add("continuebutton");
  removeModal.textContent = "Continue Playing";
